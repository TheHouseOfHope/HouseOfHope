    ...
    return (
      <div className="space-y-2 max-w-2xl">
        ...
        <p className="text-destructive text-sm">
          ...
          (<code className="text-xs">dotnet run</code> ΓåÆ http://localhost:4000), then refresh.
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {loading ? (
          ...
          <>
            ...
            <StatCard title="Donations (this month)" value={`Γé▒${Math.round(monthlyDonationsTotal).toLocaleString()}`} icon={<DollarSign className="h-6 w-6" />} description="Monetary & estimated value" />
                  ...
                  {recentDonations.map(d => (
                    <div key={d.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                      ...
                      <span className="text-sm font-semibold text-primary">
                        {d.amount != null ? `${d.currency === 'USD' ? '$' : 'Γé▒'}${d.amount.toLocaleString()}` : d.type}
                      </span>
                    </div>
                  ))}
                </div>
